...
export function CreateAccountModal({ accounts, dictionary }: Readonly<CreateAccountModalProps>) {
  ...
  return (
    <dialog ref={dialogRef} onClose={handleDialogClose} aria-labelledby="create-account-title"
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-2xl shadow-2xl max-h-[90vh] overflow-y-auto"
        style={{ transform: isVisible ? 'scale(1) translateY(0)' : 'scale(0.93) translateY(12px)', opacity: isVisible ? 1 : 0, transition: isVisible ? 'transform 280ms cubic-bezier(0.34, 1.56, 0.64, 1), opacity 200ms cubic-bezier(0.4, 0, 0.2, 1)' : 'transform 200ms cubic-bezier(0.4, 0, 0.2, 1), opacity 180ms cubic-bezier(0.4, 0, 0.2, 1)' }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-5 space-y-4" noValidate>
          ...
          {isPocket ? (
            <>
              ...
              <div>
                <label htmlFor="acc-name" className={labelCls}>{get(dictionary, 'pocketName')}</label>
                <input id="acc-name" type="text" autoComplete="off" placeholder={get(dictionary, 'pocketNamePlaceholder')}
              ...
              <div>
                <label htmlFor="acc-rate" className={labelCls}>{get(dictionary, 'interestRatePocket')}</label>
                ...
                <p id="acc-rate-hint" className="mt-1 text-xs text-slate-500">{get(dictionary, 'interestRateHint')}</p>
              </div>
            </>
          ) : (
            <>
              <div>
                <label htmlFor="acc-name" className={labelCls}>{get(dictionary, 'accountName')}</label>
                <input id="acc-name" type="text" autoComplete="off" placeholder={get(dictionary, 'accountNamePlaceholder')}
                  aria-invalid={!!errors.name} className={inputCls} {...register('name')} />
                {errors.name && <p role="alert" className="mt-1 text-xs text-red-400">{errors.name.message}</p>}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label htmlFor="acc-type" className={labelCls}>{get(dictionary, 'accountType')}</label>
                  <select id="acc-type" aria-invalid={!!errors.type} className={selectCls} {...register('type')}>
                    <option value="">—</option>
                    {ACCOUNT_TYPES.map((t) => (
                      <option key={t} value={t} className="bg-slate-800">{get(dictionary, `types.${t}`)}</option>
                    ))}
                  </select>
                  {errors.type && <p role="alert" className="mt-1 text-xs text-red-400">{errors.type.message}</p>}
                </div>
                <div>
                  <label htmlFor="acc-currency" className={labelCls}>{get(dictionary, 'currency')}</label>
              ...
              <div>
                <label htmlFor="acc-balance" className={labelCls}>{get(dictionary, 'initialBalance')}</label>
              ...
              {showRate && (
                <div>
                  <label htmlFor="acc-rate" className={labelCls}>{get(dictionary, 'interestRate')}</label>
                  <FormattedNumericInput id="acc-rate" value={rateHundredths} suffix="%" maxValue={MAX_RATE}
                    onChange={(v) => { setRateHundredths(v); setValue('interestRateEA', v / 100); }}
                    aria-describedby="acc-rate-hint"
                    className={`${inputCls} font-mono tabular-nums`} />
